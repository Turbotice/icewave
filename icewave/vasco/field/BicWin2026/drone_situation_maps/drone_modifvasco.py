...
def get_record(drone,srtfile,date='0213',year='2026'):
...
def get_records(date,jpg=True,year='2026'):
...
def get_srtfiles(date,year='2026'):
    srtfiles = {}
    base = df.find_path(year=year,date=date)
    print(base)
    for key in drones:
...
def convert_flightrecords(date,year = '2026'):
...
def get_csvfiles(date,year = '2026', disk=disk):
    csvfiles = {}
    base = df.find_path(year=year,date=date)
    base = f'{disk}/data/'
    print(base)
