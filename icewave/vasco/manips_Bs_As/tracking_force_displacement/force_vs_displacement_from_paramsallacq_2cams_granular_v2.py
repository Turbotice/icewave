...
def load_force_vs_frames_1balance(csv_filename_px_balance=csv_filename_px_single_balance,plot=False):
    frame_frac = data_csv[idx, 12]  
...
if os.path.exists(f'{path2dailydir}/{acqname_found}/pixel_balance_vs_frame.csv'):
    ...
    dict_results = load_force_vs_frames_1balance()
else:
    ...
    for i in range(len(list_dg_sur_dpx)):
        dict_results[f'balance_{list_balances_maxweights[i]}g'] = load_force_vs_frames_1balance(csv_filename_px_balance=f'pixel_balance_vs_frame_{list_balances_maxweights[i]}g.csv',plot=True)
