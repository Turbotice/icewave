...
def load_force_vs_frames_1balance_from_dirpath(dirpath, dg_sur_dpx=1, csv_filename_px_balance='pixel_balance_vs_frame.csv',
                                  csv_filename_px_ref = 'pixel_vs_frame_ref.csv', disk='D:',plot=False):

    path2csv = f'{dirpath}/{csv_filename_px_balance}'
    ...
    # si la ref bouge un peu, on soustrait la ref :
    if os.path.exists(f'{dirpath}/{csv_filename_px_ref}'):
        pd_data_pxref = pd.read_csv(f'{dirpath}/{csv_filename_px_ref}')
        x_px_ref = pd_data_pxref['X'].to_numpy()
        frames_ref = pd_data_pxref['Y'].to_numpy()
        print(x_px_ref)
    ...
    if csv_filename_px_balance=='pixel_balance_vs_frame.csv':
        # means we gave the dg_sur_dpx as input of the function
        pass
        ...
        if not match:
            print('Dont find the dg_sur_dpx file !!')
        nombre = match.group(1)
        txt_file_dg_sur_dpx = f"{dirpath}/dg_sur_dpx_{nombre}g.txt"
        dg_sur_dpx = float(np.loadtxt(txt_file_dg_sur_dpx))
    else:
        print('csv filename px balance ??')
        dg_sur_dpx = np.nan
