...
def load_force_displacement_curve(idx=0, disk='D:', data_csv=data_csv,dates_to_correct_fil=dates_to_correct_fil, Slopes_correction_fil=Slopes_correction_fil, plot=True):
...
for i in range(N):
    ...
    if (qualite_fit[i]!='ok') and (qualite_fit[i]!='moyen'):
        ...
        print(qualite_fit[i])
        arr_h_avg[i], arr_h_std[i], arr_E[i], arr_Eerr[i],arr_prefix_filename_thickness[i],arr_dates[i] = load_force_displacement_curve(idx=i,plot=True)
...
#%%
"""
# plot versus temps pour casser
nb_frames2break = df['frame_frac'].values - df['frame_force_application'].values
freq_acq = df['freq_acq_Hz'].values
time2break_sec = nb_frames2break/freq_acq
print('nb frames to break:', nb_frames2break)
print('time (sec) to break:', nb_frames2break/freq_acq)

plt.figure()
plt.plot(time2break_sec[mask],arr_E[mask],'o')

# regarder aussi en ft de taille grains
