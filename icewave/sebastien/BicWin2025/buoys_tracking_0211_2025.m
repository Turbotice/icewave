...
base = ['T:/PIV_images/' date '/Drones/' drone_ID '/' exp_ID '/'];
...
% define file2save
base_save = ['U:/Data/' date '/Drones/' drone_ID '/matData/' exp_ID '/'];
file2save = [base_save 'Buoys_tracking_pix_' date '_' drone_ID '_' exp_ID '.mat'];

% set values of RGB channel
R0 = 58/256; % value on canal R
G0 = 13/256; % value on canal G 
B0 = 24/256; % value on canal B 
RGB0 = [R0,G0,B0];
...
SE = strel('square',2);
min_area = 6; % minimal area of detected objects 
...
for i0 = 14147:20411
    % i0 = 14147;
    % disp(tiffFullPaths{i0})
    img = imread(tiffFullPaths{i0});
    
    img_crop = img(:,minx:end,:);
    % figure(1);
    % imshow(img_crop)
    
    new_table = detect_buoys(img_crop,RGB0,threshold,SE,min_distance);
    ...
    for j = 1:length(new_table.Area)
        if new_table.Area(j) > min_area
            ...
            xc = new_table.Centroid(j,1) + minx;
