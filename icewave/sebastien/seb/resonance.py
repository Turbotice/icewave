...
def demod(y,f0,facq=26.5):
    ...
    """ Computes the demodulation product of a signal y by a signal of frequency f0 
    facq corresponds to the acquisition frequency """
...
def cart2pol(x, y):
    """ Converts two arrays x and y in cartesian coordinates to polar coordinates (rho, phi) """
...
def compute_dephasage(Xfilt,Zfilt,facq=26.5,window_size = 2**6, title='',display=False):
    """ Computes the dephasage between two signals Xfilt and Zfilt, both acquired at an acquisition frequency facq.
    Dephasage is computed for several frequencies, continuoulsy distributed between 0 Hz and 10 Hz 
    Each signal is cut into smaller signals of size window_size. The dephasage"""
    ...
    # size of the signal
    N = len(Xfilt)
    # gets closest power 2 to N 
    Np = int(2**np.floor(np.log2(N)))

    n=int(Np/window_size)
    nb=int(Np/n*2-1)
    print(n)
    dt = 1/facq
    
    # array of frequency at which we will proceed demodulation
    ...
    for j,f0 in enumerate(frange):
        # we cut the signal into windows of size window_size and proceed the demodulation for each window
        for i in range(n):
    ...
    # on moyenne sur l'ensemble des fenêtres
    Zf = np.mean(C,axis=0) #np.mean(np.exp(1j*Theta),axis=0)#on moyenne sur les angles
    
    # Thetaf corresponds à la phase de chaque 
