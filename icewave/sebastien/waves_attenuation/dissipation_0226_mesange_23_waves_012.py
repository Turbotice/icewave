# -*- coding: utf-8 -*-
"""
Created on Thu Apr 24 10:01:16 2025
...
indices_freq = np.where(np.logical_and(FFT_spectrum['f'] > frequency_range[0],FFT_spectrum['f'] < frequency_range[1]))[0]

# create array of frequencies and amplitude 
freq = FFT_spectrum['f'][indices_freq]
...
closest_harmonic[np.where(dict_f['k']<0.35)] = 1

mask = np.where(np.logical_and(closest_harmonic == 3,dict_f['k'] < 0.63))
