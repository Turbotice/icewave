...
fig_folder = f'{main_path}Results/Drones/attenuation/'
...
for date in dates :
    path2data = f'{main_path}{date}/Drones/'
    # search all .mat file 
    filelist = glob.glob(path2data + '**/Results/main_results*.pkl',recursive = True)
...
dates = ['0226']
colors = {'10-waves_005':'tab:blue','23-waves_012':'tab:orange'}
for date in dates :
    for key_drone in data[date].keys():
        for exp_ID in data[date][key_drone].keys():
            x = data[date][key_drone][exp_ID]['FFT_spectrum']['f']
            y = data[date][key_drone][exp_ID]['FFT_spectrum']['TF_spectrum']
            current_color = colors[exp_ID]
            ax.loglog(x,y,'-',color = current_color,label = exp_ID)
...
figname = fig_folder + 'Superposition_TF_spectrum_0226'
...
for date in dates:
    for key_drone in data[date].keys():
        for exp_ID in data[date][key_drone].keys():
            x = data[date][key_drone][exp_ID][key_attenuation]['k']
            y = data[date][key_drone][exp_ID][key_attenuation]['f']
            hw = data[date][key_drone][exp_ID][key_attenuation]['hw']
            current_label = r'$h_w = {x:.2f}$ m / {ID}'.format(x = hw,ID = exp_ID)
            current_color = colors[exp_ID]
            ax.plot(x,y,'o',color = current_color,label = current_label)
...
figname = fig_folder + 'Superposition_dispersion_relation_0226'
plt.savefig(figname + '.pdf', bbox_inches='tight')
plt.savefig(figname + '.svg', bbox_inches='tight')
plt.savefig(figname + '.png', bbox_inches='tight')
...
for date in dates :
    for key_drone in data[date].keys():
        for exp_ID in data[date][key_drone].keys():
            ...
            beta = current_att['power_law']['beta']
            B = current_att['power_law']['B']
            current_label =  r'$\alpha(f) = ' + f'{B:.2f}'+ 'f^{' + f'{beta:.3f}' + '}$' + f'/ {exp_ID}'
            
            ax.plot(x,y,'o', color = colors[exp_ID],label = current_label)
            power_fit = powerfun(f_fit,[beta, B ])
            ax.loglog(f_fit,power_fit,'--',color = colors[exp_ID])
...
figname = fig_folder + 'Superposition_attenuation_law_0226'
plt.savefig(figname + '.pdf', bbox_inches='tight')
plt.savefig(figname + '.svg', bbox_inches='tight')
plt.savefig(figname + '.png', bbox_inches='tight')
    ...
    for key_drone in data[date].keys():
        for exp_ID in data[date][key_drone].keys():
            ...
            if key_word in keywords_list:
                ...
                ax.plot(x,y,'o',color = current_color,markeredgecolor = 'k',markersize = marker_size_plot,label = current_label)
                power_fit = powerfun(f_fit,[beta , B])
                ax.plot(f_fit,power_fit,'--',color = current_color)
