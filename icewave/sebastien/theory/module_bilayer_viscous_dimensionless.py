...
def dim_numbers(params,g = 9.81):
    ...
    omega = np.float64(2*np.pi)*params[0]
    k_0 = gk(params[0])
    gamma = params[1]*k_0
...
def define_M_norm(kappa,dimensionless):
    """ Create matrix for dispersion relation 
    Inputs : - kappa, float, wavevector value
             - dimensionless, parameters used to build matrix M (gamma,delta_1,delta_2,r)
...
def newton_raphson(params, k0, tol=1e-5, max_iter=100, step_derivative = 1e-6,threshold_derivative = 1e-12):
...
def get_coeffs(kappa,dimensionless,display=False):
    ...
    M_hat = define_M_hat(kappa, dimensionless)#modify for testing (SP)
