# -*- coding: utf-8 -*-
"""
Created on Thu May 28 13:48:51 2026

@author: sebas

Apply Markov Chain Monte Carlo Algorithm to compute E, nu, h_ice, rho_ice from 
phase velocity c_QS0, c_SH0 and dispersion relation k_QS(omega). 
...
for direction in ([1,2]):
    ...
    file2load =  f'{path2data}{year}_{date}_acq{acqu_numb}disp_QS_dir{direction}.pkl'
    with open(file2load,'rb') as pfile:
        f_mode,k_mode = pickle.load(pfile)
        s[key_dir]['f'] = f_mode
        s[key_dir]['k'] = k_mode

#%% Compute C_longi and C_shear from both directions

C_shear = 0.5*(s['dir1']['C_shear'] + s['dir2']['C_shear'])
C_longi = 0.5*(s['dir1']['C_longi'] + s['dir2']['C_longi'])

#%% Interpolate k_QS dispersion relation over both directions
...
def wavenumbers_stein_squire( rho_ice, h, H, E, nu,freq,c_w,rho_w,equation):
    ...
    for kf in range(len(freq)):
        ...
        if omeg == 0:
            ...
            func[func.imag != 0] = -1
            func = func.real
            a = np.where(np.diff(np.signbit(func)))[0][0] # indices of the array k at which func(k) = 0
            idx_zero[kf] = a
...
def simulated_annealing(delta_param0, T0, T0param, Tmin, Tminparam, X, MIN_param0, MAX_param0, 
                        data, isave, equation, file2save, fig_save_path):
...
# If T0param != Tminparam : we are performing a Metropolis algorithm with a progressive step, 
# we can make smaller step after each iteration 
...
# set file to save MCMC results
file2save = f'{path2data}{year}_{date}_acq{acqu_numb}_MCMC_inversion_bidir_results.pkl'
# set path to save MCMC figures
fig_save_path = f'{path2data}MCMC_results/'
...
I,T_critic, it_critic, X, likelihood ,misfit_accepted  = simulated_annealing(delta_param0,T0,T0param,
                    Tmin,Tminparam,X,MIN_param0,MAX_param0,data,isave,equation, file2save, fig_save_path)
