...
base = 'Y:';
folder = [ base '/Banquise/Sebastien/Calibration_intrinseque/' ...
    'Calib_PMMH/bernache/calib_video/Binarisation/'];
...
%% Save detected subgrids in .mat file

filename = 'matrix_points_test_1218.mat';
save(filename,'Mp')

%% Load detected subgrids 
...
for i=1:length(Mp)
    ...
    hold on
    plot(xc,yc,'o')
    d = sqrt((Xout-xc).^2+(Yout-yc).^2); % distance to center 
...
figure(2)
for k = 1:4
    plot(Mp(i0).m.corners{k}(1),Mp(i0).m.corners{k}(2),'ks')
    hold on 
    text(Mp(i0).m.corners{k}(1) + 2,Mp(i0).m.corners{k}(2) + 2,num2str(k))
end

%%

for i=16:length(Mp) % loop over all subgrids
    disp(i)
    m = Mp(i).m;
    Xout = m.X;
    Yout = m.Y;

    figure(10) % plot subgrid
    hold off
    plot(Xout,Yout,'x')
